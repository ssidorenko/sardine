...
class OSCInHandler(BaseHandler):
    ...
    def attach(self, address: str, function: Callable, watch: bool = False, argscheme=None):
        """
        ...
        osc_method(address, function, argscheme=OSCARG_DATAUNPACK if argscheme is None else argscheme)
        if watch:
            self.watch(address)
