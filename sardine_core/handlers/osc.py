...
class OSCHandler(Sender):
    ...
    def send_raw(self, address: str, message: list, nudge=False) -> None:
        ...
        """
        if nudge:
            self.call_timed_with_nudge(self.env.clock.shifted_time, self._send, address, message)
        else:
            self._send(address, message)

    def send_raw_bundle(self, messages: list, nudge=False) -> None:
        if nudge:
            self.call_timed_with_nudge(self.env.clock.shifted_time, self._send_bundle, messages)
        else:
